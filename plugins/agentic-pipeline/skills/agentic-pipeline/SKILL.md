...
This skill exists to override ad hoc model habits with project-local operating logic. Do not rely on an agent "remembering" to behave correctly. Make the required behavior explicit, observable, and rejectable.
...
- `blocked` means the agent may inspect, edit docs/scripts, or produce a remediation plan, but must not claim build/run/deploy/test success for the blocked branch.
...
- Reject any agent result that does not report which guidance sources it followed and what preflight evidence it used.
...
5. Spawn discovery and planning agents.
...
8. Supervise background agents without blocking.
