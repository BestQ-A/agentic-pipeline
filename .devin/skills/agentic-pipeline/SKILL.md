...
A pipeline is not a loop merely because a subagent can be rerun. Before calling a workflow a loop, prove that it contains the six operating parts and that each part has evidence.
...
- A missing `verification_check` is a nodding loop. The same subagent that generated the output must not be the final judge of that output.
...
- Grow loops in this order: first one finding end to end, then stronger discovery, then stronger verification, then scheduling, then parallelism. A loop earns more parallel subagents only after its evaluator has caught real or fixture-backed failures.
...
2. Capture repeatable project evidence.
   ...
   - Use the report to identify existing `AGENTS.md`, `CLAUDE.md`, `.codex/agents`, `.agents/skills`, `.hermes/skills`, `.devin/skills`, `.devin/agents`, `.codex/skills`, `rules`, scripts, tests, package/build commands, loop artifacts, and retained context.
